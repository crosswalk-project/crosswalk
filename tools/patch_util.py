...
class PatchInfo(object):
  ...
  def parse_stream(self, stream):
    ...
    nextfileno = 0
    nexthunkno = 0    # even if index starts with 0 user messages
                      # number hunks from 1
      ...
      if hunkbody:
        ...
        if re.match(r"^[- \+\\]", line):
          ...
          warning("invalid hunk no.%d at %d for target file %s"
              % (nexthunkno, lineno+1, self.target[nextfileno-1]))
        ...
        if hunkactual["linessrc"] > hunkinfo.linessrc \
            or hunkactual["linestgt"] > hunkinfo.linestgt:
          warning("extra hunk no.%d lines at %d for target %s"
              % (nexthunkno, lineno+1, self.target[nextfileno-1]))
          # add hunk status node
          self.hunks[nextfileno-1].append(hunkinfo.copy())
          self.hunks[nextfileno-1][nexthunkno-1]["invalid"] = True
          ...
          hunkbody = False
          hunkskip = True
        elif hunkinfo.linessrc == hunkactual["linessrc"] \
            and hunkinfo.linestgt == hunkactual["linestgt"]:
          self.hunks[nextfileno-1].append(hunkinfo.copy())
          # switch to hunkskip state
          hunkbody = False
          hunkskip = True
      ...
      if hunkskip:
        ...
        if match:
          ...
          if debugmode and len(self.source) > 0:
            debug("- %2d hunks for %s"
                % (len(self.hunks[nextfileno-1]), self.source[nextfileno-1]))
      ...
      if filenames:
        if line.startswith("--- "):
          ...
          if nextfileno in self.target:
            warning("skipping invalid patch - double target at line %d"
                % lineno)
            del self.source[nextfileno]
            ...
            match = re.match(re_filename, line)
            if not match:
              warning("skipping invalid patch - no target filename at line %d"
                  % lineno)
              # switch back to header state
              filenames = False
              header = True
      ...
      if hunkhead:
        ...
        if not match:
          if nextfileno-1 not in self.hunks:
            warning("skipping invalid patch with no hunks for file %s"
                % self.target[nextfileno-1])
    ...
    info("total files: %d  total hunks: %d"
        % (len(self.source), sum(len(hset) for hset in self.hunks)))
  ...
  def apply(self, root_directory = None):
    ...
    for fileno, filename in enumerate(self.source):
      ...
      if not exists(f2patch):
        # if the patch contains a single hunk at position 0 consider
        # it's a new file
        if len(self.hunks[fileno]) == 1 and self.hunks[fileno][0].startsrc == 0:
          ...
          if len(hunklines) > 0:
            ...
            # Create the directory firstly in the file path if it doesn't exist
      ...
      for lineno, line in enumerate(f2fp):
        ...
        if lineno+1 < hunk.startsrc+len(hunkfind)-1:
          if line.rstrip("\r\n") == hunkfind[hunklineno]:
            ...
            debug("hunk no.%d doesn't match source file %s"
                % (hunkno+1, filename))
        ...
        if lineno+1 == hunk.startsrc+len(hunkfind)-1:
          debug("file %s hunk no.%d -- is ready to be patched"
              % (filename, hunkno+1))
          hunkno += 1
          validhunks += 1
          if hunkno < len(self.hunks[fileno]):
        ...
        if hunkno < len(self.hunks[fileno]) and \
            (len(self.hunks[fileno]) != 1 \
                or self.hunks[fileno][0].startsrc != 0):
