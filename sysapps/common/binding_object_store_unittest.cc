...
TEST(XWalkSysAppsBindingObjectStoreTest, AddBindingObject) {
  ...
  EXPECT_FALSE(store->HasObject("foobar1"));
  EXPECT_FALSE(store->HasObject("foobar2"));
  EXPECT_FALSE(store->HasObject("foobar3"));
  EXPECT_FALSE(store->HasObject("foobar4"));
  ...
  EXPECT_TRUE(store->HasObject("foobar1"));
  EXPECT_TRUE(store->HasObject("foobar2"));
  EXPECT_TRUE(store->HasObject("foobar3"));
  EXPECT_TRUE(store->HasObject("foobar4"));
...
TEST(XWalkSysAppsBindingObjectStoreTest, OnDestroyObject) {
  ...
  EXPECT_TRUE(handler.HandleFunction(
          CreateFunctionInfo("destroyObject", "foobar1")));
  EXPECT_EQ(BindingObjectTest::instance_count(), 3);

  EXPECT_TRUE(handler.HandleFunction(
          CreateFunctionInfo("destroyObject", "foobar2")));
