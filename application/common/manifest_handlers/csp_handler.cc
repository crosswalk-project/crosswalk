...
bool CSPHandler::Parse(scoped_refptr<ApplicationData> application,
  ...
  std::vector<std::string> policies = base::SplitString(policies_str, directive_separator, base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
  for (size_t i = 0; i < policies.size(); ++i) {
    ...
    std::vector<std::string> directive_value = base::SplitString(directive_value_str, base::kWhitespaceASCII, base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
