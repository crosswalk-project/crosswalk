...
class Application : public base::RefCountedThreadSafe<Application> {
 public:
  ...
  // In a few special circumstances, we want to create an Application and give
  // it an explicit id. Most consumers should just use the other Create()
  // method.
  static scoped_refptr<Application> Create(const base::FilePath& path,
      Manifest::Location location,
      const base::DictionaryValue& value,
  ...
  const xwalk_application::Manifest* manifest() const {
 ...
 private:
  ...
  static bool InitApplicationID(xwalk_application::Manifest* manifest,
