...
PrinterQuery::PrinterQuery(int render_process_id, int render_view_id)
    : worker_(new PrintJobWorker(render_process_id, render_view_id, this)),
      ...
      cookie_(printing::PrintSettings::NewCookie()),
      last_status_(printing::PrintingContext::FAILED) {
...
void PrinterQuery::GetSettingsDone(const printing::PrintSettings& new_settings,
                                   printing::PrintingContext::Result result) {
  is_print_dialog_box_shown_ = false;
  last_status_ = result;
  if (result != printing::PrintingContext::FAILED) {
...
const printing::PrintSettings& PrinterQuery::settings() const {
  return settings_;
}
