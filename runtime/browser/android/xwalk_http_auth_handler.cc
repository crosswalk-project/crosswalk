...
bool XWalkHttpAuthHandler::HandleOnUIThread(content::WebContents* web_contents) {
  DCHECK(web_contents);
  DCHECK(content::BrowserThread::CurrentlyOn(content::BrowserThread::UI));
  XWalkContent* xwalk_content = XWalkContent::FromWebContents(web_contents);
  return xwalk_content->contents_client_bridge()->OnReceivedHttpAuthRequest(http_auth_handler_, host_, realm_);
}
...
XWalkHttpAuthHandlerBase* XWalkHttpAuthHandlerBase::Create(
    ...
    net::AuthChallengeInfo* auth_info,
    bool first_auth_attempt) {
  return new XWalkHttpAuthHandler(login_delegate, auth_info, first_auth_attempt);
}
