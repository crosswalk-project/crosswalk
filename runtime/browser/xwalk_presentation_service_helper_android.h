...
class PresentationSessionAndroid
...
class PresentationFrameAndroid
    : public PresentationFrame,
      public XWalkPresentationHost::SessionObserver {
 public:
  explicit PresentationFrameAndroid(const RenderFrameHostId& render_frame_host_id);
  ~PresentationFrameAndroid() override;

  void OnPresentationClosed(int render_process_id,
    int render_frame_id);
};
