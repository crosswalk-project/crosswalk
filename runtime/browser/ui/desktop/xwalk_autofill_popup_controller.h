...
#ifndef XWALK_RUNTIME_BROWSER_UI_DESKTOP_XWALK_AUTOFILL_POPUP_CONTROLLER_H_
#define XWALK_RUNTIME_BROWSER_UI_DESKTOP_XWALK_AUTOFILL_POPUP_CONTROLLER_H_
...
class XWalkAutofillPopupView;
class XWalkPopupController;

// This class is a controller for an XWalkAutofillPopupView.
class XWalkAutofillPopupController {
 public:
  // Creates a new |XWalkAutofillPopupController|, or reuses |previous| if the
  ...
  static base::WeakPtr<XWalkAutofillPopupController> GetOrCreate(
      base::WeakPtr<XWalkAutofillPopupController> previous,
  ...
  void SetSelectionAtPoint(const gfx::Point& point);
  void SelectionCleared();
  size_t GetLineCount() const;

  const gfx::Rect& popup_bounds() const;

  const base::string16& GetElidedLabelAt(size_t row) const;
  const autofill::Suggestion& GetSuggestionAt(size_t row) const;
  const base::string16& GetElidedValueAt(size_t row) const;
 ...
 private:
  ...
  bool RemoveSuggestion(int list_index);
  ...
  base::WeakPtr<XWalkAutofillPopupController> GetWeakPtr();
  ...
  XWalkAutofillPopupView* view_;  // Weak reference.
  ...
  base::WeakPtrFactory<XWalkAutofillPopupController> weak_ptr_factory_;
};

}  // namespace xwalk

#endif  // XWALK_RUNTIME_BROWSER_UI_DESKTOP_XWALK_AUTOFILL_POPUP_CONTROLLER_H_
