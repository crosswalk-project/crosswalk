...
DownloadBarView::DownloadBarView(NativeAppWindow* app_view)
    : download_image_(nullptr),
      ellipsis_label_(nullptr),
      close_button_(nullptr) {
  set_id(NativeAppWindowDesktop::VIEW_ID_DOWNLOAD_BAR);
  parent_ = static_cast<NativeAppWindowDesktop*>(app_view);
...
void DownloadBarView::Layout() {
  ...
  download_image_->SetBounds(kDownloadImageHorizontalPadding, y,
                             image_size.width(), image_size.height());

  gfx::Size btn_size = close_button_->GetPreferredSize();
  x = this->width() - btn_size.width() - kCloseButtonRightPadding;
  y = (height - btn_size.height()) / 2;
  close_button_->SetBounds(x, y, btn_size.width(), btn_size.height());

  int max_item_x = x - kCloseButtonLeftPadding - kMaxItemTextWidth;
  x = width;
  for (auto& item : download_item_views_) {
    if (x > max_item_x) {
...
void DownloadBarView::AddDownloadItem(
  ...
  if (!parent_->GetViewByID(NativeAppWindowDesktop::VIEW_ID_DOWNLOAD_BAR)) {
