...
gfx::Image LoadImageFromFilePath(const base::FilePath& filename) {
  ...
  if (base::EndsWith(filename.value(), kPNGFormat, base::CompareCase::INSENSITIVE_ASCII)) {
    std::string contents;
  ...
  if (base::EndsWith(filename.value(), kJPGFormat, base::CompareCase::INSENSITIVE_ASCII) ||
      base::EndsWith(filename.value(), kJPEGFormat, base::CompareCase::INSENSITIVE_ASCII)) {
    std::string contents;
    base::ReadFileToString(filename, &contents);
  ...
  if (base::EndsWith(filename.value(), kICOFormat, base::CompareCase::INSENSITIVE_ASCII)) {
#if defined(OS_WIN)
