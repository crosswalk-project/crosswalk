    ...
    private void initXWalkContent(Context context, AttributeSet attrs) {
        ...
        if (!CommandLine.getInstance().hasSwitch("disable-xwalk-extensions")) {
            ...
            try {
                ...
                Log.w("XWalkViewInternal::initXWalkContent()", "Class BuiltinXWalkExtensions is not found. check the gyp flag: disable_android_buildin_extensions.");  
            } catch (Exception e) {
                Log.w("XWalkViewInternal::initXWalkContent()", "error in load buildin extensions.");
            }
        } else {
