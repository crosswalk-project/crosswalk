...
void ExecuteSessionCommandOnSessionThread(
  ...
  if (status.IsError() && session->xwalk) {
    if (!session->quit && session->xwalk->HasCrashedWebView()) {
      session->quit = true;
      ...
      Status status_tmp = session->xwalk->GetWebViewIds(&web_view_ids);
      if (status_tmp.IsError() && status_tmp.code() != kXwalkNotReachable) {
        status.AddDetails(
    ...
    if (status.IsError()) {
      status.AddDetails(
          "Session info: xwalk=" + session->xwalk->GetVersion());
    }
  }
