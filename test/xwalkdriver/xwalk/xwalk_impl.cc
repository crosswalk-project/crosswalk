...
Status XwalkImpl::GetWebViewById(const std::string& id, WebView** web_view) {
  for (WebViewList::iterator it = web_views_.begin();
       it != web_views_.end(); ++it) {
    if ((*it)->GetId() == id) {
      *web_view = (*it).get();
...
Status XwalkImpl::CloseWebView(const std::string& id) {
...
Status XwalkImpl::ActivateWebView(const std::string& id) {
