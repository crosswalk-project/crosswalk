...
#include <minizip/ioapi.h>  // NOLINT(build/include_order)
#else
...
unsigned long ReadZipBuffer(void* opaque, void* /*stream*/, void* buf, unsigned long size) {  // NOLINT
  ZipBuffer* buffer = static_cast<ZipBuffer*>(opaque);
  ...
  if (!buffer || !buffer->data || !remaining_bytes)
    return 0;
  size = std::min(size, static_cast<unsigned long>(remaining_bytes));  // NOLINT
...
unsigned long WriteZipBuffer(void* /*opaque*/,  // NOLINT
                     void* /*stream*/,
                     const void* /*buf*/,
...
long GetOffsetOfZipBuffer(void* opaque, void* /*stream*/) {  // NOLINT
  ZipBuffer* buffer = static_cast<ZipBuffer*>(opaque);
  if (!buffer)
    return -1;
  return static_cast<long>(buffer->offset);  // NOLINT
}

// Moves the current offset to the specified position.
long SeekZipBuffer(void* opaque, void* /*stream*/, unsigned long offset, int origin) {  // NOLINT
  ZipBuffer* buffer = static_cast<ZipBuffer*>(opaque);
  if (!buffer)
    return -1;
