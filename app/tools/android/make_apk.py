...
COMPRESSED_LIBRARY = 'libxwalkcoreCompressed.so'
...
def ContainsCompressedLibrary(path):
  return os.path.isfile(os.path.join(path, COMPRESSED_LIBRARY))
...
def CopyNativeLibrary(native_path, library_path, arch):
...
def Execution(options, app_info):
  ...
  if options.mode == 'embedded':
    ...
    if contains_library(native_path):
      copy_library(native_path, library_path, arch)
    else:
...
def MakeCompressedLibrary(lib_dir):
  # For Lite, compressed library is generated by default.
  return False


def MakeNativeLibrary(lib_dir):
  # Use lzma to decompress the compressed library.
  compressed_library = os.path.join(lib_dir, COMPRESSED_LIBRARY)
  compressed_suffix = os.path.join(lib_dir, NATIVE_LIBRARY + '.lzma')
  os.rename(compressed_library, compressed_suffix)
  RunCommand(['lzma', '-d', compressed_suffix])
  return True
