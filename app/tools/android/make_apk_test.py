    ...
    if os.path.exists(target_dir):
      ...
      if not os.path.exists(test_des_dir):
        ...
        if not options.external_extensions:
          options.external_extensions = os.path.join(target_dir, os.pardir, "lib.java")
        ...
        try:
          for e_name in os.listdir(extensions_dir):
            ...
            src_file = os.path.join(options.external_extensions, "%s.jar" % e_name)
            dst_file = os.path.join(e_dir, "%s.jar" % e_name)
            print dst_file
            print src_file
            shutil.copyfile(src_file, dst_file)
        except:
          print("Failed to get external extensions, related tests may fail. \
                 Remember to compile 'xwalk_external_extensions.'")
